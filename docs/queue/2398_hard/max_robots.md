...
__通过率36%徘徊__ 的第2398号题 事实上只是想知道答题人懂不懂得一个基本意识：__审题__

一旦有好好审题 才会确信 __单调队列、滑动窗口、前缀和__ 三宝能合而为一拿下


### 永远只带著自然数的chargeTimes和runningCosts
输入有两项阵列：```chargeTimes```和```runningCosts``` 双方长度皆为$n$

第$i$个索引上分别是第$i$台机器人的充电时间和运行成本
...
__$总开销 = k个机器人中最大的充电时间 + k * 全部k个机器人们运行成本总和$__

因此题目问的是 在总开销不超过预算的前提下 最多能 __连续__ 抓多少台机器人同时运行

注意有 __连续__ 这个形容词 __它不接受同时抓第$i$台和第$i + 2$台 但不抓第$i + 1$台的跳跳虎模式__

看到这儿 我们就会察觉一个巧妙的事实：若$0 \le i <= j <= l < n$

一旦连抓第$i$到第$j$台机器人时 发现这j + 1 - i台机器人的总开销超过预算

那我们抓第$i$到第$l$台机器人时 一样会看见这l + 1 - i台机器人总开销超过预算 甚至更严重

为什么呢？第$i$到第$j$台机器人中最大的chargeTime <= 第$i$到第$l$台机器人中最大的chargeTime __前者是后者的子集__

于是公式中的『k个机器人中最大的充电时间』没法下降

又我刚才说过运行成本都是自然数 __因此第$i$到第$l$台机器人运行成本总和 > 第$i$到第$j$台机器人运行成本总和__

k随著机器人越抓越多只会上涨 既然三个子项都不可能下降 那总开销就不可能下降了

于是我们有个结论：一旦发生了第$i$到第$j$台机器人总开销超预算 __非得放弃第$i$台机器人__ 才有机会压回预算内
...
反映的是当前抓了第```startIdx```到第```endIdx```台 总计```endIdx``` + 1 - ```startIdx```这么多台机器人在测算总开销
...
只要队列尾的充电时间 <= 第```endIdx```台机器人的充电时间 队列尾自然失去作用 弹走就对啰
...
这样便能每次都拥有正确的窗口内运行成本总和

由此可见遍历```endIdx```过程中 一旦```startIdx```不再上升 那么窗口内的机器人自然不会产生超预算的总开销

这群机器人总数便是 __```endIdx``` + 1 - ```startIdx```__ 如此多啰 (1)

在弹滑动窗口的设计时 我们有说是可能发生```startIdx``` __不停递增 直到 ```startIdx``` > ```endIdx```收手__
...
__真的就是好好看清楚题目下方的Constraints 联想到在如此限制下 哪些数据结构能最快地保证正确性 时空复杂度双双O(n)__
...
### 延伸问题
回过头看 题目那句必须 __连续取机器人__ 的叙述 各位觉得是条罗嗦限制 还是其实是代码能极速化的核心呢🤓
