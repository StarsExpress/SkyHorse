...
靠著$nums[j + 1]$这项 硬是让$sum(nums[i: j + 2]) * (j + 2 - i) < k$
...
若右方索引$j$引发$sum(nums[i: j + 1]) * (j + 1 - i) > k$

那么更右方的索引$l$ 同样会让$sum(nums[i: l + 1]) * (l + 1 - i) > k$成立

而且超出的程度还更严重 __因此前述的担忧叫做：杞人忧天🤣__


### 爽快！那就直接上吧
我们放心大胆地采取这道方针：__一旦$sum(nums[i: j + 1]) * (j + 1 - i) > k$发生了__

__就必须递增$i$ 直到$sum(nums[i: j + 1]) * (j + 1 - i) \leq k$终于成立 或者$i > j$为止__
...
### [随时要能联想自己写过的类似题解](https://starsexpress.github.io/SkyHorse/docs/hash/2963_hard/good_partitions)
⬆️事实上本题的思路 就和第2963道难题是一样的 甚至还是简化版

__第2963那题必须追踪窗口最大值 我们第2302题不用～～__

__因此能顺利节省了空间复杂度O(n)的单调递减队列__ 靠两个指针代表窗口左右端

再来一个叫做```subarraySum```的变量追踪$sum(nums[i: j + 1])$

![Prefix Sum Sliding Window_Efficiency](2302_efficiency.png)
__顺利简化到仅剩O(1)的空间复杂度而已～～时间复杂度仍是线性的O(n) 20行不到的代码😄__


### 插播：如果输入的阵列有正有负 那该如何
[Algo Monster](https://algo.monster/liteproblems/2302) 对第2302题使用的 __Binary Search__

__就是如果输入的数组有正有负时 能照样确保正确性的作法 代价是时间复杂度涨到O(nlogn) 空间复杂度涨到O(n)__


### 因此审题非常重要 Bayes定理不是说说而已
如果只知道输入的数组全是整数 但不清楚正负情况

那我们只有$P(Y)$这样的先验概率做判读信息 得像Algo Monster开启比较麻烦的二分查找

但题目说的好清楚 只给自然数 __这便是似然$P(X | Y)$似然到手了__
...
### 延伸问题
当在扫描右边界索引为$j$的子数组时 看见左边界$i$递增成为了$j + 1$ 我们能因此对$nums[j]$下什么样的结论呢😏
