...
## 永远的春娇先攻
老惯例了 LeetCode都嘛给春娇走第一步 __女士优先精神__
...
(1). 如果$D_{n - 2} > D_{n - 1}$ 那么春娇会放弃一条龙方案

改为合并只包含到第$n - 2$颗石子 第$n - 1$颗留在台上
...
__我的净分差还大于直接手拿$D_{n - 1}$的分差 好处更多耶__』

(2). 反过来 若$D_{n - 2} \leq D_{n - 1}$ 那春娇就继续一条龙方案

同时我们还得回到推导过的那条数学式

$D_{n - 2} = \text{sum}(stones[:n - 1]) - D_{n - 1}$ 由此可知
...
其中索引范围是$1 \leq i \leq n - 2$ 索引范围为何长这样：

(1). __索引范围没有零__

__是因为题目规定每次都得合并起码两颗还在台上的石头__

(2). 至于索引范围又为何没包含$n - 1$呢？因为我们的

__Base Case恰恰是开局把全部石头合并起来 游戏结束__
...
因此在做状态转移时 __建议从较大索引往较小索引遍历__

[省得要搞额外的DFS和cache来 导致$O(n)$空间复杂度](https://algo.monster/liteproblems/1872)
...
直接开一个叫做```max_score_diff```的变量记载$D_i$即可
